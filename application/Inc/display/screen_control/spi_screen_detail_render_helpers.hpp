...
void ScreenDetailRender_List(ST7789_Handle* lcd, const char* title, const char* const* labels, uint8_t count, uint8_t index, uint8_t selectedConfigIndex, const ScreenUiStyle& style, uint16_t itemH = SPI_SCREEN_DETAIL_ITEM_H, bool animActive = false, int animDir = 0, uint32_t animStartMs = 0u, uint32_t nowMs = 0u, const bool* disabledItems = nullptr);
