...
/* Microsoft OS descriptor hooks used by XInput-compatible binding on Windows. */
...
static const uint8_t s_ms_os_string_desc[] = {
    ...
    '1', 0x00u, '0', 0x00u, '0', 0x00u, MS_VENDOR_CODE, 0x00u
};

static const uint8_t s_ms_compat_id_desc[] = {
    0x28u, 0x00u, 0x00u, 0x00u, /* dwLength = 40 */
    0x00u, 0x01u,               /* bcdVersion = 1.00 */
    0x04u, 0x00u,               /* wIndex = 0x0004 */
    0x01u,                      /* bCount */
    0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, /* reserved */
    0x00u,                      /* bFirstInterfaceNumber */
    0x01u,                      /* reserved */
    'X', 'U', 'S', 'B', '1', '0', 0x00u, 0x00u,       /* compatibleID */
    0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, /* subCompatibleID */
    0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u          /* reserved */
};

/* Minimal Extended Properties descriptor (no properties). */
static const uint8_t s_ms_ext_prop_desc[] = {
    0x0Au, 0x00u, 0x00u, 0x00u, /* dwLength = 10 */
    0x00u, 0x01u,               /* bcdVersion = 1.00 */
    0x05u, 0x00u,               /* wIndex = 0x0005 */
    0x00u, 0x00u                /* wCount = 0 */
};

/* 115200-8N1 default line coding for CDC ACM. */
static uint8_t s_cdc_line_coding[7] = {0x00u, 0xC2u, 0x01u, 0x00u, 0x00u, 0x00u, 0x08u};
    ...
    if( intflag & USBHS_UDIF_TRANSFER )
    {
        ...
        if( !(intst & USBHS_UDIS_EP_DIR )) // SETUP/OUT Transaction
        {
            switch( endp_num )
            {
                            ...
                            if (( USBHS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_CLASS)
                              {
                                switch( USBHS_SetupReqCode )
                                {
                                    case CDC_GET_LINE_CODING:
                                        len = 7u;
                                        memcpy(USBHS_EP0_Buf, s_cdc_line_coding, len);
                                        break;
                                    ...
                                    default:
                                        /*
                                         * XInput host stacks may issue vendor/class probes.
                                         * Ack unknown class requests with ZLP instead of STALL.
                                         */
                                        len = 0;
                                        break;
                                }
                              }
                            else if (( USBHS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_VENDOR)
                            {
                                if (((USBHS_SetupReqType & DEF_UEP_IN) != 0u) &&
                                    (USBHS_SetupReqCode == MS_VENDOR_CODE) &&
                                    (USBHS_SetupReqIndex == MS_OS_DESC_COMPAT_ID_INDEX))
                                {
                                    pUSBHS_Descr = s_ms_compat_id_desc;
                                    len = (uint16_t)sizeof(s_ms_compat_id_desc);
                                ...
                                else if (((USBHS_SetupReqType & DEF_UEP_IN) != 0u) &&
                                         (USBHS_SetupReqCode == MS_VENDOR_CODE) &&
                                         (USBHS_SetupReqIndex == MS_OS_DESC_EXT_PROP_INDEX))
                                {
                                    pUSBHS_Descr = s_ms_ext_prop_desc;
                                    len = (uint16_t)sizeof(s_ms_ext_prop_desc);
                                    if( USBHS_SetupReqLen > len )
                                    {
                                        USBHS_SetupReqLen = len;
                                    }
                                ...
                                {
                                    /*
                                     * Avoid early enumeration failure on vendor probes:
                                     * return zero-filled IN data or ZLP for OUT.
                                     */
                                    if(( USBHS_SetupReqType & DEF_UEP_IN ) != 0u)
                                    {
                                        len = (USBHS_SetupReqLen >= DEF_USBD_UEP0_SIZE) ? DEF_USBD_UEP0_SIZE : USBHS_SetupReqLen;
                                    ...
                                    switch( (uint8_t)(USBHS_SetupReqValue>>8) )
                                    {
                                        ...
                                        case USB_DESCR_TYP_STRING:
                                            switch( (uint8_t)(USBHS_SetupReqValue&0xFF) )
                                            {
                                                ...
                                                case DEF_STRING_DESC_SERN:
                                                    ...
                                                    break;
                                                case 0x04:
                                                    pUSBHS_Descr = MyXsm3Info;
                                                    len = (uint16_t)MyXsm3Info[0];
                                                    break;

                                                case MS_OS_STRING_INDEX:
                                                    pUSBHS_Descr = s_ms_os_string_desc;
                                                    len = (uint16_t)sizeof(s_ms_os_string_desc);
                                                    break;
                                        ...
                                        case USB_DESCR_TYP_REPORT:
                                            if( USBHS_DevSpeed == USBHS_SPEED_HIGH )
                                            {
                                                ...
                                                len = XINPUT_TELEMETRY_REPORT_DESC_LEN;
                                            }
                                            else
                                            {
                ...
                case DEF_UEP4:
                    if ( R8_U2EP4_RX_CTRL & USBHS_UEP_R_TOG_MATCH )
                    {
        ...
        else
        {
          /* data-in stage processing */
            switch ( endp_num )
            {
                ...
                case DEF_UEP5:
                    R16_U2EP5_T_LEN = 0;
                    R8_U2EP5_TX_CTRL ^= USBHS_UEP_T_TOG_DATA1;
