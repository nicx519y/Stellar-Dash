...
- 当前正式产品名为 **XORA**（全大写）。面向用户的界面、中英文文案、提示、标题、无障碍标签和新文档统一使用 XORA，不再使用 HBox / HBOX / Hitbox 作为产品名。
- 旧名称仅可保留在兼容性技术标识或历史引用中，例如仓库路径、源码符号、构建宏、命令、协议值、加密上下文、设备识别值和存储键；不得为更名批量替换这些标识或改变既有通信、产物和数据兼容性。
...
本节适用于 XORA 开发任务；模块指令提供定向入口，硬件安全红线、必需验收和用户明确的暂停要求优先。第三方库的上游全量检查清单不自动成为 XORA 每次改动的要求。
