...
static const uint8_t * xbone_get_string_descriptor(int index) {
	if ( index == 3 ) {
		// 从STM32芯片的内部唯一ID生成序列号
		// STM32H7有一个96位(12字节)的唯一ID，存储在特定的地址
		uint32_t *stm32_unique_id = (uint32_t *)0x1FF1E800; // STM32H7系列的UID地址
		
		// 复制12字节的唯一ID到序列号缓冲区
		for (int i = 0; i < 3; i++) {
			((uint32_t *)uniqueSerial)[i] = stm32_unique_id[i];
		}
		return uniqueSerial;
