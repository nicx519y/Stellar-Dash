...
第八轮将 USB/Maintenance 角色 ACK 后的固定等待改为被动就绪脉冲观察。
只有读取 ACK 时已经确认 W_INT 释放，且随后看见应用就绪脉冲拉低、释放并保持
高电平至少 2 ms，才提前继续。ACK 释放不明确、脉冲错过或未完成时，保留原有
150 ms 等待上限；等待期间不产生 SPI 时钟。CAPS 活性校验、失败重试、CH585
固件及 IAP 不变。RF 角色不走此等待。

21 项定向主机测试通过，包括生产等待逻辑的 36 组边界组合、16 个已有启动重叠
场景、USB 传输兼容与握手/计时契约；完整无锁构建耗时约 51 秒，ELF/manifest
检查通过。USB 状态码表达式补充 uint8_t 显式转换，以通过原生编译器严格检查；
改动前 HEAD 可复现同一类型告警。冻结契约另有一个已有失败：tools/hbox.py
哈希与冻结记录不同；工作区该文件与 HEAD 相同，本轮没有修改它或契约哈希。

已按原入口完成 app A 及原诊断 bootloader 烧录和回读。证据目录为
`.hbox/boot-profile-role-ready-20260925-023423/`，保存旧镜像、构建/测试/烧录日志、
manifest、RAM 原始记录和 comparison.json。烧录后启动及两次普通软件复位的
记录均完整（v4、80 条、序号 7/8/9），USB Input 路径的数据如下：

| 区间 | 上轮三次 / ms | 本轮三次 / ms | 中位数减少 / ms |
|---|---|---|---|
| 板级完成至主循环 | 1130 / 1130 / 1142 | 1103 / 1103 / 1115 | 27 |
| Input 状态进入 | 726 / 726 / 738 | 699 / 699 / 711 | 27 |
| CH585 剩余启动总段 | 653 / 653 / 653 | 626 / 626 / 626 | 27 |
| CH585 角色交接 | 263 / 263 / 263 | 236 / 236 / 236 | 27 |
| CH585 就绪提示等待 | 390 / 390 / 390 | 390 / 390 / 390 | 0 |
| USB prepare | 43 / 43 / 59 | 43 / 43 / 59 | 0 |
| USB connect | 12 / 12 / 8 | 12 / 12 / 8 | 0 |

上述父子区间不能累加。本轮应用初始化中位数改善约 2.4%，bootloader 仍为
231 ms；SHA 约 152.89 ms，校验范围不变。记录器 boot 稳定主频下校准开销为
0.708–0.850 us/次，应用记录及缓存清理开销未另测，也未从区间中估算扣除。
保留无身份开发路径的 attestation_failed 标志和正常认证节点缺失提示；这些数据
不能作为完整认证启动基线。尚无本轮冷启动数据或按键/USB 人工验收，三次复位
不作为正式 P95 或稳定性结论。未进行 RF 回归/采样、CH585 固件写入；未修改
任何保护位或锁定状态。
