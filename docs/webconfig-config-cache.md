...
保存前失效直接写入的模块缓存；只有具有配套版本的完整成功回包才能更新缓存。无 ACK、部分回包及更新单条宏等场景下保留失效状态。Profile 保存保留关联列表与宏的历史正文及原始指纹，重连时必须经过设备清单核对才能复用；重命名、兼容路径写入内嵌宏等真实变化会因指纹不一致而补读，普通按键、触发或灯光保存不会无条件清除这两项。导入完成命令使整份缓存失效。未保存草稿、预览与结果未知的写入不会被当作已确认配置。
