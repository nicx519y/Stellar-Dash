...
bool load(Config &config){(void)config;return true;} bool save(Config &config){(void)config;return true;} bool reset(Config &config){(void)config;return true;} bool fromStorage(Config &config){(void)config;return true;}
}
...
ADCManager &ADCManager::getInstance(){static ADCManager i;if(i.mapping_.id[0]=='\0')i.resetForContractTest();return i;} void ADCManager::resetForContractTest(){mapping_={};strcpy(mapping_.id,"mapping-default");strcpy(mapping_.name,"Default");mapping_.length=3;mapping_.step=.1f;mapping_.originalValues[0]=100;mapping_.originalValues[1]=200;mapping_.originalValues[2]=300;default_="mapping-default";} std::vector<ADCValuesMapping*> ADCManager::getMappingList(){return {&mapping_};} const ADCValuesMapping *ADCManager::getMapping(const char *id)const{return id&&strcmp(id,mapping_.id)==0?&mapping_:nullptr;} ADCBtnsError ADCManager::createADCMapping(const char *name,size_t length,float_t step){if(!name||!length)return ADCBtnsError::INVALID_PARAMS;strncpy(mapping_.name,name,sizeof(mapping_.name)-1);mapping_.length=std::min(length,(size_t)MAX_ADC_VALUES_LENGTH);mapping_.step=step;return ADCBtnsError::SUCCESS;} ADCBtnsError ADCManager::removeADCMapping(const char *id){return id?ADCBtnsError::SUCCESS:ADCBtnsError::INVALID_PARAMS;} ADCBtnsError ADCManager::renameADCMapping(const char *id,const char *name){if(!id||!name)return ADCBtnsError::INVALID_PARAMS;strncpy(mapping_.name,name,sizeof(mapping_.name)-1);return ADCBtnsError::SUCCESS;} ADCBtnsError ADCManager::setDefaultMapping(const char *id){if(!id)return ADCBtnsError::INVALID_PARAMS;default_=id;return ADCBtnsError::SUCCESS;} std::string ADCManager::getDefaultMapping()const{return default_;}
ADCBtnsMarker &ADCBtnsMarker::getInstance(){static ADCBtnsMarker i;return i;} ADCBtnsError ADCBtnsMarker::setup(const char *id){if(!id)return ADCBtnsError::INVALID_PARAMS;strncpy(step_.id,id,sizeof(step_.id)-1);step_.is_marking=true;return ADCBtnsError::SUCCESS;} ADCBtnsError ADCBtnsMarker::step(){if(!step_.is_marking)return ADCBtnsError::NOT_MARKING;++step_.index;return ADCBtnsError::SUCCESS;} void ADCBtnsMarker::reset(){step_.is_marking=false;} cJSON *ADCBtnsMarker::getStepInfoJSON()const{cJSON*o=cJSON_CreateObject();cJSON_AddStringToObject(o,"id",step_.id);cJSON_AddBoolToObject(o,"isMarking",step_.is_marking);cJSON_AddNumberToObject(o,"index",step_.index);return o;}
