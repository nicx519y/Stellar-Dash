...
size_t rfm_protocol_encode(const rfm_proto_frame_t *frame, uint8_t *out, size_t out_cap)
{
    ...
    if ((frame == 0) || (out == 0) || (frame->payload_len > RFM_PROTO_MAX_PAYLOAD)) {
        return 0u;
    }

    total_len = RFM_PROTO_HEADER_SIZE + frame->payload_len + RFM_PROTO_TAIL_CHECK_SIZE;
    ...
    for (i = 0u; i < frame->payload_len; ++i) {
        out[RFM_PROTO_HEADER_SIZE + i] = frame->payload[i];
    }
...
bool rfm_protocol_decode(const uint8_t *raw, size_t raw_len, rfm_proto_frame_t *out)
{
    ...
    if ((raw == 0) || (out == 0) || (raw_len < (RFM_PROTO_HEADER_SIZE + RFM_PROTO_TAIL_CHECK_SIZE))) {
        return false;
    }

    payload_len = raw_len - RFM_PROTO_HEADER_SIZE - RFM_PROTO_TAIL_CHECK_SIZE;
    ...
    for (i = 0u; i < payload_len; ++i) {
        out->payload[i] = raw[RFM_PROTO_HEADER_SIZE + i];
    }

    return (out->hdr.version == RFM_PROTO_VERSION);
}
