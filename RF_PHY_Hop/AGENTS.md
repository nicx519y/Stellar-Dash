...
2026-09-22 v32：用户实机报告 v31 接收缺额约33%、延迟表大量缺源，v31 不通过验收。已有日志确认辅助队列15/15、累计丢弃31322；ELF确认新增计时作用域返回结构体调用Flash memcpy。RX 0x1932消除此隐式调用、计时保留周期/RAM阈值并后台换算、辅助改为有界连续单步消费、报表编码移出锁、首份报告独占事件关联。仅编译/静态检查，测试用例只编译未运行，未采样/烧录，恢复效果待实机。见 ../docs/RF_RX_REGRESSION_V32_20260922.md。

2026-09-22 v30/v31：已交付独立RX计时基线0x1930与直通流水线0x1931，RXP1累计阶段统计。v31普通短包在接收回调构造20B报告，4待发+1在途，USB完成接续，满/500us积压恢复最新；保留中立/代次取消，后台辅助每步1项。空口v5不变，正常TX测试源默认关闭，5B/7B bench独立目录。已编译和静态检查，测试只编译/编写未运行，无采集/烧录，125us实机未验收。见 ../docs/RF_RX_PIPELINE_V31_20260922.md。

2026-09-22 RX 125us 审查：v29 不能作为持续不同输入 8K 能力验收。旧 0x1927 日志 RF 约7719/s、主循环短输入接纳约1447/s、混合合并计数约6066/s，不能把后两者当理论性能上限或丢键数。latest/FIFO/prepared 有覆盖，EP2 完成后等主循环装载；g_demo_edge_drop 没有递增，零值无证明力。尚缺完整分段 CPU/等待耗时。仅代码/已有日志审查，未继续固件改动/回归/采集/烧录，详见 ../docs/RF_RX_PIPELINE_AUDIT_20260922.md。

2026-09-22 v29：仅 RX 修复相同输入的重复包更新 input_serial、导致主循环解码反复失效的问题。序号改为按 18 键+测量 tag+radio generation 的状态修订，保留提交门禁；事件建档/解码/构包放 RAM。RX build 0x1929，空口 v5，兼容 v27/v28 TX 和 v27 monitor。已有日志 RX 最大 2987us；只编译/静态检查，未回归/新采集/烧录，不能宣称所有尖峰已消除。见 ../docs/RF_RX_LATENCY_V29_20260922.md。
