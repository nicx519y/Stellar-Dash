...
static inline uint32_t rfh_access_address_from_seed(uint32_t seed)
{
    ...
    seed ^= seed >> 17;
    seed ^= seed << 5;
    address = seed ^ 0xA5A55A5AUL;

    if((address == RFH_LINK_ACCESS_ADDRESS_DEFAULT) ||
       (rfh_access_address_valid(address) == 0u))
    {
        address = RFH_LINK_ACCESS_ADDRESS_DEFAULT ^ 0x13579BDFUL;
    }
    if(rfh_access_address_valid(address) == 0u)
    {
        address = 0x6D35B8C9UL;
    }
    return address;
}
