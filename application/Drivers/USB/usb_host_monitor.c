    ...
    for (uint8_t addr = 1; addr <= CFG_TUH_DEVICE_MAX; addr++) {
        if (device_info[addr].connected || tuh_connected(addr)) {
            ...
            if (!physical_connected && device_info[addr].connected) {
                // USB_DBG("[USB监控] 状态不一致：物理已断开但设备 #%d 仍标记为连接\r\n", addr);
                
                if (!device_info[addr].umount_cb_called) {
                    // USB_DBG("[USB监控] 卸载回调未曾触发，尝试直接修复设备 #%d\r\n", addr);
...
void usb_host_monitor_hid_unmounted(uint8_t dev_addr, uint8_t idx) {
    ...
    USB_DBG("[USB监控] HID设备卸载: addr=%d, idx=%d\r\n", dev_addr, idx);
...
void usb_host_monitor_hid_closed(uint8_t dev_addr) {
    USB_DBG("[USB监控] HID设备关闭函数调用: addr=%d\r\n", dev_addr);
}
...
void usb_host_monitor_task(void) {
    ...
    if (now - last_check > 500) {
            ...
            if (last_physical_connected && !physical_connected) {
                ...
                for (uint8_t addr = 1; addr <= CFG_TUH_DEVICE_MAX; addr++) {
                    if (tuh_connected(addr)) {
                        USB_DBG("[USB监控] 警告: 设备 #%d 在物理断开后仍标记为已连接!\r\n", addr);
                        found_stale = true;
                        ...
                        USB_DBG("[USB监控] 尝试修复：手动触发设备 #%d 的移除流程\r\n", addr);
                        
                        // 方法1：直接调用卸载回调（可能不完全解决底层状态问题）
                        for (uint8_t i = 0; i < CFG_TUH_HID; i++) {
                            // 检查此设备是否有HID接口
                            if (tuh_hid_mounted(addr, i)) {
                                USB_DBG("[USB监控] 设备 #%d 接口 #%d 仍然标记为已挂载，手动调用卸载回调\r\n", addr, i);
                                // 手动调用卸载回调
        ...
        // 持续断开状态下的检查
            ...
            if (stale_device_time > 0 && (now - stale_device_time > 500)) {
                bool needed_cleanup = false;
                
                USB_DBG("[USB监控] 断开状态持续500ms，检查是否有悬空设备需要强制清理...\r\n");
                for (uint8_t addr = 1; addr <= CFG_TUH_DEVICE_MAX; addr++) {
                    if (tuh_connected(addr)) {
                        USB_DBG("[USB监控] 悬空设备 #%d 需要强制清理\r\n", addr);
                        needed_cleanup = true;
        ...
        // 持续连接状态下的检查 - 可能存在软件连接状态不一致
            ...
            // 连接恢复5秒后，检查设备状态是否正确
            if (now - connection_recovery_time > 5000) {
                USB_DBG("[USB监控] 连接恢复5秒后，进行设备状态一致性检查\r\n");
...
void usb_host_monitor_check_stale_devices(void) {
    ...
    if (!physical_connected) {
        ...
        for (uint8_t addr = 1; addr <= CFG_TUH_DEVICE_MAX; addr++) {
            if (tuh_connected(addr)) {
                USB_DBG("[USB监控] 状态不一致: 设备 #%d 标记为已连接，但物理连接已断开\r\n", addr);
            }
        }
    }
}
    ...
    for (uint8_t addr = 1; addr <= CFG_TUH_DEVICE_MAX; addr++) {
        for (uint8_t idx = 0; idx < CFG_TUH_HID; idx++) {
            if (tuh_hid_mounted(addr, idx)) {
                USB_DBG("[USB监控] HID接口已挂载: 设备=%d, 接口=%d\r\n", addr, idx);
...
bool usb_host_monitor_force_device_removal(uint8_t dev_addr) {
    ...
    uint32_t now = HAL_GetTick();
    if (now < 1000) {
        USB_DBG("[USB监控] 系统启动阶段（%lums）不执行设备移除\r\n", now);
        return false;
    }

    USB_DBG("[USB监控] 强制移除设备 #%d\r\n", dev_addr);
...
bool usb_host_monitor_force_reset_bus(uint8_t rhport) {
    USB_DBG("[USB监控] 尝试强制重置USB总线 端口 #%d\r\n", rhport);
    
    // 1. 先尝试清理所有设备状态
    for (uint8_t addr = 1; addr <= CFG_TUH_DEVICE_MAX; addr++) {
        if (tuh_connected(addr)) {
            USB_DBG("[USB监控] 总线重置前移除设备 #%d\r\n", addr);
            usb_host_monitor_force_device_removal(addr);
        }
    }
...
void monitor_interrupt_status(uint8_t rhport, uint32_t int_status) {
    ...
    if (int_status != last_int_status) {
        if (int_status & (1U << 29)) { // DISCONNINT
            USB_DBG("[USB监控] 检测到断开中断 (DISCONNINT), rhport=%d\r\n", rhport);
        }
...
void monitor_device_disconnect_event(uint8_t rhport) {
    USB_DBG("[USB监控] 设备断开事件触发 (rhport=%d)\r\n", rhport);
    ...
    if (!physical_connected) {
        ...
        for (uint8_t addr = 1; addr <= CFG_TUH_DEVICE_MAX; addr++) {
            if (tuh_connected(addr)) {
                ...
                USB_DBG("[USB监控] 断开事件检测：设备 #%d 仍标记为已连接\r\n", addr);
                
                // 只在物理断开但设备标记为连接的情况下发送设备移除事件
                // 这是最小且安全的处理方式
                USB_DBG("[USB监控] 尝试标记设备 #%d 为已断开\r\n", addr);
